...
At other times, you need to work the concent of the title into the adminition text, as in the following example:
...
```
Use the `ls` command to list files. You can provide a wildcard to listy only the files that fit this wildcard.
...
Note: if you need a really complicated list element with elements such as paragraphs and code blocks, sometimes it is genuinely hard to encure the correct formatting in AsciiDoc by precise use of the `+` character. An alternative is the use of an [open block](https://docs.asciidoctor.org/asciidoc/latest/blocks/open-blocks/), denoted by `--` characters on separate lines. The first paragraph of a list cannot be a part of an open block, so you still must have a `+` on a separate line undr it, followed by the open block. The following example is _correct_ and the second element of the list includes three paragraphs:
...
In some of these cases the line break is unnecessary for the context. In these cases cimply remove the `+` character, as in the following example:
...
Sometimes the `+` character is used to join paragraphs into a list element and is correctpy placed on its own lone, but there is an empty line or a comment before or after the `+` character.
